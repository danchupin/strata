// strata-inventory walks every bucket's InventoryConfiguration entries and
// produces (manifest.json, CSV.gz) reports into the configured target buckets.
// Leader-elected via internal/leader so only one inventory worker is active
// per Strata cluster.
package main
...
func main() {
	...
	w, err := inventory.New(inventory.Config{
		...
		Data:     dataBackend,
		Logger:   logger,
		Interval: parseDuration("STRATA_INVENTORY_INTERVAL", 5*time.Minute),
		Region:   envOr("STRATA_INVENTORY_REGION", "default"),
	})
	if err != nil {
	...
	metrics.Register()
	go func() {
		listen := envOr("STRATA_INVENTORY_METRICS_LISTEN", ":9105")
		mux := http.NewServeMux()
		mux.Handle("/metrics", metrics.Handler())
		logger.Info("inventory: metrics", "listen", listen)
	...
	if locker == nil {
		logger.Warn("inventory: leader election disabled (no distributed locker for this meta backend)",
	...
	session := &leader.Session{Locker: locker, Name: "inventory", Holder: leader.DefaultHolder()}
	for ctx.Err() == nil {
