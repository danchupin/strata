...
var knownWorkers = []knownWorker{
	...
	{"notify", []string{
		"STRATA_NOTIFY_TARGETS — comma-separated <type>:<arn>=<spec> targets (required)",
		"STRATA_NOTIFY_INTERVAL (default 5s) — drain tick interval",
		"STRATA_NOTIFY_MAX_RETRIES (default 6) — attempts before DLQ",
		"STRATA_NOTIFY_BACKOFF_BASE (default 1s) — first retry delay; doubles per attempt",
		"STRATA_NOTIFY_POLL_LIMIT (default 100) — max events drained per bucket per tick",
	}},
	{"replicator", []string{
		"STRATA_REPLICATOR_INTERVAL (default 5s) — drain tick interval",
		"STRATA_REPLICATOR_MAX_RETRIES (default 6) — attempts before FAILED",
		"STRATA_REPLICATOR_BACKOFF_BASE (default 1s) — first retry delay; doubles per attempt",
		"STRATA_REPLICATOR_POLL_LIMIT (default 100) — max events drained per bucket per tick",
