...
- `admin-api/` — flat index of every admin endpoint (method, path, audit verb, summary) with cross-links into the interactive viewer. _(authored in a follow-on cycle.)_
- `s3-api/` — table of supported S3 operations with handler `file:line` pointers and AWS-divergence notes. _(authored in a follow-on cycle.)_
