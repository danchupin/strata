...
- [ ] `internal/serverapp/serverapp.go` registers the handler under
      ...
      not collide with bucket names. Bucket name `console` is already
      reserved by the existing validator (US to come if not — verify
      and add to the bucket-name validator if missing)
...
- [ ] New endpoint `GET /admin/v1/cluster/nodes` returns:
      ...
      `cluster_nodes` table (Cassandra + TiKV in lockstep) every
      10 s with TTL 30 s. Replica reads the table to assemble the
      response. Leadership info comes from the existing
      `internal/leader` lease tables (one row per worker)
...
- `internal/meta.Store` — existing, reused for cluster nodes table +
  bucket list + objects
