...
// ReconcileConfig tunes the leader-elected reconcile worker (US-002
// metadata-data-reconcile). Interval is the poll cadence between RunOnce
// passes that drain queued reconcile jobs; CheckpointEvery is the number of
// chunks scanned between cursor+counter watermark persists (smaller = more
// resumable, more meta load). The pool-scan rate limit is read separately
// from STRATA_RECONCILE_SCAN_RATE (objects/sec; rados.ScanRateFromEnv).
...
func (c *Config) clampWorkers() {
	...
	c.Workers.Reconcile.Interval = clampDuration("workers.reconcile.interval", c.Workers.Reconcile.Interval, time.Second, time.Hour)
	c.Workers.Reconcile.CheckpointEvery = clampInt("workers.reconcile.checkpoint_every", c.Workers.Reconcile.CheckpointEvery, 1, 100000)
}
