...
// CaseClusterRegistry exercises the cluster-registry CRUD surface (US-001).
// Backends wire this directly in their own test files once they ship a real
// implementation (memory in US-001, cassandra in US-002, tikv in US-003).
func CaseClusterRegistry(t *testing.T, s meta.Store) {
	ctx := context.Background()

	// List on an empty registry returns nil slice, not an error.
	got, err := s.ListClusters(ctx)
	if err != nil {
		t.Fatalf("ListClusters empty: %v", err)
	}
	if got != nil {
		t.Fatalf("ListClusters empty: got %d entries, want nil slice", len(got))
	}

	// GetCluster on missing id returns ErrClusterNotFound.
	if _, err := s.GetCluster(ctx, "nope"); err != meta.ErrClusterNotFound {
		t.Fatalf("GetCluster missing: got %v want ErrClusterNotFound", err)
	}

	// Insert two rows.
	c1 := &meta.ClusterRegistryEntry{ID: "beta", Backend: "rados", Spec: []byte(`{"name":"beta"}`)}
	if err := s.PutCluster(ctx, c1); err != nil {
		t.Fatalf("PutCluster c1: %v", err)
	}
	if c1.Version == 0 {
		t.Fatalf("PutCluster c1 did not stamp Version: %+v", c1)
	}
	if c1.CreatedAt.IsZero() || c1.UpdatedAt.IsZero() {
		t.Fatalf("PutCluster c1 did not stamp timestamps: %+v", c1)
	}
	c2 := &meta.ClusterRegistryEntry{ID: "alpha", Backend: "rados", Spec: []byte(`{"name":"alpha"}`)}
	if err := s.PutCluster(ctx, c2); err != nil {
		t.Fatalf("PutCluster c2: %v", err)
	}

	// List returns both rows sorted by ID ascending.
	got, err = s.ListClusters(ctx)
	if err != nil {
		t.Fatalf("ListClusters two: %v", err)
	}
	if len(got) != 2 {
		t.Fatalf("ListClusters two: got %d rows, want 2", len(got))
	}
	if got[0].ID != "alpha" || got[1].ID != "beta" {
		t.Fatalf("ListClusters ordering: got [%s, %s] want [alpha, beta]", got[0].ID, got[1].ID)
	}
	if string(got[0].Spec) != `{"name":"alpha"}` {
		t.Fatalf("ListClusters spec round-trip: got %q", string(got[0].Spec))
	}

	// Get returns one row.
	one, err := s.GetCluster(ctx, "beta")
	if err != nil {
		t.Fatalf("GetCluster beta: %v", err)
	}
	if one.Backend != "rados" || string(one.Spec) != `{"name":"beta"}` {
		t.Fatalf("GetCluster beta payload: %+v", one)
	}
	if one.Version != 1 {
		t.Fatalf("GetCluster beta version: got %d, want 1", one.Version)
	}

	// Stale-version update returns ErrClusterVersionMismatch.
	stale := &meta.ClusterRegistryEntry{ID: "beta", Backend: "rados", Spec: []byte(`{"name":"beta2"}`), Version: 0}
	if err := s.PutCluster(ctx, stale); err != meta.ErrClusterVersionMismatch {
		t.Fatalf("PutCluster stale: got %v want ErrClusterVersionMismatch", err)
	}

	// Update with matching Version succeeds + bumps Version.
	upd := &meta.ClusterRegistryEntry{ID: "beta", Backend: "rados", Spec: []byte(`{"name":"beta2"}`), Version: one.Version}
	prevUpdated := one.UpdatedAt
	time.Sleep(time.Millisecond)
	if err := s.PutCluster(ctx, upd); err != nil {
		t.Fatalf("PutCluster update: %v", err)
	}
	if upd.Version != one.Version+1 {
		t.Fatalf("PutCluster update version: got %d, want %d", upd.Version, one.Version+1)
	}
	reread, err := s.GetCluster(ctx, "beta")
	if err != nil {
		t.Fatalf("GetCluster post-update: %v", err)
	}
	if string(reread.Spec) != `{"name":"beta2"}` {
		t.Fatalf("post-update spec: got %q", string(reread.Spec))
	}
	if !reread.UpdatedAt.After(prevUpdated) {
		t.Fatalf("post-update UpdatedAt not advanced: %v vs %v", reread.UpdatedAt, prevUpdated)
	}

	// Delete + Get returns ErrClusterNotFound.
	if err := s.DeleteCluster(ctx, "beta"); err != nil {
		t.Fatalf("DeleteCluster: %v", err)
	}
	if _, err := s.GetCluster(ctx, "beta"); err != meta.ErrClusterNotFound {
		t.Fatalf("GetCluster after delete: got %v want ErrClusterNotFound", err)
	}
	if err := s.DeleteCluster(ctx, "beta"); err != meta.ErrClusterNotFound {
		t.Fatalf("DeleteCluster idempotent: got %v want ErrClusterNotFound", err)
	}

	// Verify alpha survives.
	got, err = s.ListClusters(ctx)
	if err != nil || len(got) != 1 || got[0].ID != "alpha" {
		t.Fatalf("ListClusters post-delete: rows=%v err=%v", got, err)
	}
}
