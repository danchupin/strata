...
func TestWorkerSkipsDrainingTargetsAtPlanTime(t *testing.T) {
	...
	if err := m.SetClusterState(context.Background(), "c2", meta.ClusterStateEvacuating, meta.ClusterModeEvacuate, 0); err != nil {
...
func TestWorkerMovesOutOfDrainingCluster(t *testing.T) {
	...
	if err := m.SetClusterState(context.Background(), "c1", meta.ClusterStateEvacuating, meta.ClusterModeEvacuate, 0); err != nil {
		t.Fatalf("SetClusterState: %v", err)
	}
