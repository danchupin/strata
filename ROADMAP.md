...
- ~~**P1 — gc / lifecycle workers serialise inside a single goroutine; throughput cap ~50–500 ops/s.**~~ — **Done.** Phase 1 shipped via the `ralph/gc-lifecycle-scale` cycle (US-001..US-005). Bounded `errgroup` inside the elected leader (`STRATA_GC_CONCURRENCY` / `STRATA_LIFECYCLE_CONCURRENCY`, default 1, max 256) lifts per-worker throughput ~9× (gc) / ~19× (lifecycle) on the canonical lab-tikv stack. Measured on N=10000 with `STRATA_DATA_BACKEND=memory` + TiKV: gc 11108 → 100275 chunks/s (c=1 → c=256, knee at c=64 with +11 % beyond), lifecycle 485 → 9150 objects/s (c=1 → c=256, no knee inside the swept range). Recommended production default: `STRATA_GC_CONCURRENCY=64`, `STRATA_LIFECYCLE_CONCURRENCY=64` (push to 128–256 if the meta backend has headroom). Bench harness lands as `strata-admin bench-gc` + `strata-admin bench-lifecycle`; results captured in `docs/benchmarks/gc-lifecycle.md`. (commit `<pending>`)
- **P1 — gc / lifecycle Phase 2 — sharded leader-election.** Phase 1 (above) lifts the per-leader cap; the leader itself is still single-replica. For multi-replica deploys serving prod-scale churn (10k object PUTs/s × ~4 chunks/each = 40k chunks/s) the per-replica cap eventually saturates. Phase 2: shard the leader-election space so multiple replicas process disjoint slices in parallel. Scope: (a) gc gets `gc-leader-0..N-1` lease keys, each leader scans its shard via a NEW `Meta.ListGCEntriesShard(ctx, region, shardID, shardCount, batch)` API that filters on `hash(oid) % shardCount == shardID` server-side (cassandra: token-range hint, tikv: prefix `gc/<region>/<shardID>/`); (b) lifecycle gets per-bucket leases (`lifecycle-leader-<bucketID>`) so each replica owns a strict subset of buckets — no cross-bucket coordination needed because lifecycle is bucket-scoped today. Backwards-compat: `STRATA_GC_SHARDS=N` (default 1 = current behaviour); existing single-leader operators see no change. Pair with bench harness rerun on a 3-replica stack to quantify the curve. Surfaced + carried over from the gc-lifecycle-scale cycle's Phase 2 deferral.
