...
- **P2 — S3 backend per-instance lifecycle + cluster registry consumer.** The Dynamic-clusters cycle (`ralph/dynamic-clusters`) shipped the catalogue + admin API + RADOS watcher, but the S3 data backend (`internal/data/s3/backend.go`) is single-bucket-per-instance and does not consume the registry — `backend=s3` rows are persisted but never reconciled into a connection pool. Fix scope: lift `data.s3.Backend` to a multi-cluster `clusters map[string]*S3ClusterSpec` shape (mirroring the rados side); add `internal/data/s3/registry_watcher.go` that consumes `meta.ListClusters` with `backend=="s3"` rows; per-cluster SDK client construction + reconciliation on add / remove / update; wire `ClassSpec.Cluster` on the s3 path so per-storage-class routing picks up registry entries. Same admin API surface — no new endpoints. Defer the chunk-side migration story to the placement policy + rebalance worker entry below.
- ~~**P2 — Dynamic RADOS / S3 cluster registry + zero-downtime add.**~~ — **Done.** Shipped via the `ralph/dynamic-clusters` cycle (US-001..US-007). Catalogue persisted in `meta.Store` as `ClusterRegistryEntry` (memory + Cassandra `cluster_registry` table + TiKV `s/cr/<id>` prefix); admin endpoints `GET / POST / DELETE /admin/v1/storage/clusters` with format-only validation + `409 ClusterReferenced {referenced_by: [...]}` on drop-while-referenced. `internal/data/rados/registry_watcher.go` polls `meta.ListClusters` every `STRATA_CLUSTER_REGISTRY_INTERVAL` (default 30 s, range `[5 s, 5 m]`) and reconciles `Backend.clusters` under `b.mu` — added clusters lazy-dial on next traffic via existing `connFor`, removed clusters drain cached `*rados.Conn` + ioctxes. `Backend.Close(ctx)` stops the watcher off-lock before draining. Counter `strata_cluster_registry_changes_total{op=add|remove|update}` increments per reconciliation. `STRATA_RADOS_CLUSTERS` env path fully retired — registry is the single source of truth. Operator guide [Dynamic clusters](docs/site/content/best-practices/dynamic-clusters.md); S3-backend half tracked as the follow-up P2 entry above. (commit `a02fe34`)
