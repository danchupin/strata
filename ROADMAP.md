...
- **P2 — UI confusion: "Ready to deregister" chip + "Cancel deregister prep" button shown together.** After full evacuation `<DrainProgressBar>` renders the green "✓ Ready to deregister" status chip AND `<ClustersSubsection>` shows the "Cancel deregister prep" button on the same card. Two contradictory messages — chip says "drain complete, action ready" while button label suggests "do nothing yet, cancel preparation". Operator confused which action is primary. Fix scope: (a) rename button to "Restore to live (cancel evacuation)" — clarifies it's the escape hatch, not a primary action; (b) add tooltip on the chip explaining the actual primary action ("Operator edits STRATA_RADOS_CLUSTERS env to remove this cluster, then rolling restart — see operator runbook"); (c) reduce button visual weight to `variant="outline"` size="sm" so the chip dominates visually as the status indicator; (d) reorder card so status chip is above the action button, not adjacent. ~1 story.
- **P2 — Multipart-on-cluster probe is a no-op on Cassandra backend.** `internal/meta/cassandra/store.go::ListMultipartUploadsByCluster` returns `0, nil` unconditionally (comment notes "BackendUploadID is not persisted on this backend"). Half of the deregister_ready safety AND (manifest=0 AND gc_queue=0 AND multipart=0) is effectively bypass on Cassandra deploys — open multipart sessions on a draining cluster don't block deregister_ready. Operator deregisters mid-multipart → Complete fails with phantom-cluster error. Fix scope: persist `BackendUploadID.Cluster` on multipart_uploads row in Cassandra (schema-additive); query `WHERE cluster=? LIMIT 1` for the probe (will be `ALLOW FILTERING` antipattern — same denormalization treatment as gc_queue → multipart_uploads_by_cluster lookup table for production scale). Memory + TiKV backends already track this — verify their probes return non-zero when multipart open on the cluster. ~2 stories.
- **P3 — ALLOW FILTERING on `cluster` column for GC + multipart probes (denormalize into lookup tables).** drain-cleanup US-006 added `ListChunkDeletionsByCluster` + `ListMultipartUploadsByCluster` Cassandra impls using `WHERE cluster=? ALLOW FILTERING LIMIT 1`. Per CLAUDE.md cassandra gotchas, `ALLOW FILTERING` on a non-PK is antipattern at scale. Acceptable for drain-time probe (operator-driven, rare, limit=1) but warrants denormalization for production: new lookup tables `gc_entries_by_cluster (cluster, region, enqueued_at, oid)` + `multipart_uploads_by_cluster (cluster, bucket, key, upload_id)` partitioned on `(cluster)`. Maintenance: every EnqueueChunkDeletion + multipart Init/Complete/Abort dual-writes the lookup row. Probe becomes `SELECT cluster FROM <table> WHERE cluster=? LIMIT 1` — no filtering. ~3 stories.
- **P3 — Trace browser recent-list has no filter / search.** `<RecentTracesPanel>` (shipped in `ralph/drain-cleanup` US-008) renders the LRU front-N traces sortable by Started or Duration — but with 50-200 traces shown and only 4-second polling, operator can't find a specific request (e.g. "my PUT to demo-cephb 30 seconds ago"). Acceptance criteria optimised for happy-path render, missed realistic operator scenario. Fix scope: (a) extend `GET /admin/v1/diagnostics/traces` with query params `?method=`, `?status=` (Error|OK), `?path_substr=` (substring match on root_name), `?min_duration_ms=`, `?max_duration_ms=`; RingBuffer scan applies filters in-memory before pagination; (b) UI `<RecentTracesPanel>` gains filter row above the table: method dropdown, status dropdown, path substring input (debounced 250ms), min duration numeric, "Clear filters" button; filters synced to URL params for bookmark/share; empty-result state "No traces match the current filters" with hint to widen; (c) Playwright test covers filter combinations + URL persistence. ~2 stories.
