...
- **P2 — Compose `default` + `multi-cluster` profiles race for worker leases on shared Cassandra.** Running both `strata` (default service, single-cluster RADOS) AND `strata-multi` (profile multi-cluster, multi-cluster RADOS) on the same Cassandra causes lease contention on `gc-leader-N` / `rebalance-leader-N`. Whichever container starts first wins the lease — single-cluster strata grabs gc-leader-0 → its gc worker tries to delete chunks but lacks the cephb cluster, so cephb-side GC entries never drain → `deregister_ready` stays false forever. Found via operator walkthrough on the multi-cluster lab. Fix scope: (a) compose `multi-cluster` profile should NOT start the `strata` service alongside `strata-multi` — make `strata-multi` mutually exclusive with the default `strata` service via profile membership; (b) document the constraint in `deploy/docker/docker-compose.yml` + operator runbook; (c) add boot-time warning when STRATA_RADOS_CLUSTERS list differs across replicas sharing same Cassandra (detection via cluster_state row presence vs env diff); (d) smoke harness for `multi-cluster` profile asserts only strata-multi is up.
