...
- **P2 — Rebalance worker not sharded — single goroutine bottleneck on large deploys.** gc + lifecycle workers got Phase 2 treatment (per-shard leases driven by `STRATA_GC_SHARDS`, default 1, range [1, 1024]) via the `ralph/gc-lifecycle-scale-phase-2` cycle. Rebalance worker (`internal/rebalance`, leader-elected on single `rebalance-leader` lease, SkipLease=false) was never sharded — one goroutine cluster-wide scans ALL buckets sequentially, walks manifests, plans moves, executes via mover. For large deploys (10k+ buckets, M+ objects) single replica + 1m tick interval = hours-long full sweep. Fix scope: mirror the gc/lifecycle Phase 2 pattern — `STRATA_REBALANCE_SHARDS` env (default 1, range [1, 1024]); per-shard leases `rebalance-leader-0..N-1`; each replica owns subset of buckets via `fnv32a(bucketID) % STRATA_REBALANCE_SHARDS == myReplicaID` distribution gate; multi-leader bench against `lab-tikv-3` (3-replica TiKV lab) verifying ~3× aggregate throughput at SHARDS=3; ProgressTracker per-cluster aggregates merge across shards. ~5 stories. Out of scope for drain-cleanup cycle — separate `ralph/rebalance-scale-phase-2` cycle after drain-cleanup merges.
