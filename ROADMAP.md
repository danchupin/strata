...
- **P3 — Web UI — Placement + cluster surfacing.** `ralph/placement-rebalance` shipped the server-side primitives (per-bucket `Placement` policy, `cluster_state` drain sentinel, `--workers=rebalance` mover, `GET /admin/v1/clusters` + `POST /clusters/{id}/drain|undrain` + `PUT/GET/DELETE /buckets/{name}/placement` admin endpoints) but did NOT add UI surface. Operators today cannot see which cluster a chunk lives on, set placement weights, or drain a cluster without `curl`. Fix scope: (a) Cluster card / list in the Storage page (`Backend` tab) fetching `/admin/v1/clusters` — render rows `{id, state, backend, used_bytes/total_bytes from ClusterStats, last-scan chunk-count}` with Drain / Undrain action buttons; (b) `PoolStatus` wire shape adds `Cluster string` so the Pools table gains a "Cluster" column (split by cluster + class); (c) per-BucketDetail Placement edit dialog (alongside Lifecycle / CORS / Quota dialogs) with a `{cluster: weight}` slider matrix and a "reset to default" button; (d) cluster-wide draining banner when ≥1 cluster is `state=draining` (sticky top of AppShell, dismissible per-session); (e) rebalance worker progress on the cluster card (planned moves / chunks-moved-in-flight / refused counters via `strata_rebalance_*` Prometheus); (f) Playwright e2e `web/e2e/placement.spec.ts` covering set-policy + drain + rebalance flow against the `multi-cluster` compose lab.
