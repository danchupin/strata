...
- ~~**P2 — `POST /admin/v1/buckets/{name}/force-empty` leaks chunks in storage backend.**~~ — **Done.** Shipped via the `ralph/drain-cleanup` cycle (US-004). `forceEmptyDrainPage` now captures the `*meta.Object` returned by `Meta.DeleteObject` (both ListObjectVersions path and the ListObjects fallback) and routes its `Manifest.Chunks` to the GC queue via a new helper `enqueueRemovedChunks` that resolves region from `s.Region`, skips nil/empty chunk slices (delete markers + MPU residue), copies the slice defensively, and calls `Meta.EnqueueChunkDeletion` best-effort (failure is logged but never aborts the drain — meta has already committed, so a transient enqueue failure is repairable via a follow-up force-empty pass). Unit tests assert chunks land in the GC queue after drain and that zero-chunk objects skip enqueue. (commit `<pending>`)
