...
- **P3 — Precise drain-progress ETA from gateway GC tunables.** The Awaiting GC chip in `<DrainProgressBar>` ships with a static-copy tooltip ("Physical delete completes after STRATA_GC_GRACE elapses ~5m default plus the next gc worker tick.") rather than a per-deploy ETA. Operators running with non-default `STRATA_GC_GRACE` / `STRATA_GC_INTERVAL` / `STRATA_GC_BATCH_SIZE` / `STRATA_GC_CONCURRENCY` / `STRATA_GC_SHARDS` get a copy that doesn't match their cadence. Fix scope: (a) new `GET /admin/v1/gc-config` endpoint surfacing the GC tunables read from the running process env (no operator-mutable knob — env stays source of truth); (b) UI consumes the config and computes `eta_min = grace_min + ceil(gc_queue / (batch_size × shards) × interval_min)` from the response's `gc_queue_pending` counter; (c) Awaiting GC tooltip + chip label flip from static copy to live `~<N>m` ETA. Deferred until operator demand justifies the work — the static-copy tooltip already explains the wait reason qualitatively, which solves the primary operator-confusion issue. ~1 story when picked up.
