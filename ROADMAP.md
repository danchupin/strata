...
- ~~**P1 — Secure-by-default auth + `off` semantics.**~~ — **Done.** Default `STRATA_AUTH_MODE` flipped `off` → `required` so an unconfigured gateway demands valid SigV4 (was wide-open). `off` is now an explicit dev-only "no auth at all" posture — it bypasses SigV4 **and** the bucket policy/ACL gates (new `auth.AuthInfo.FullAccess`, honoured by `requireAccess` + `requireACL`), fixing the prior footgun where `off` left requests anonymous and 403'd writes to console-created buckets. Boot logs a `WARN` when `off` is active; dev paths (`make run-memory`/`run-cassandra`) opt into it explicitly. Full auth-mode matrix tests added (`internal/s3api/auth_mode_test.go`: off=full-access, optional public-read vs public-read-write anon writes, signed-non-owner denied, bad-signature denied across optional+required). Fixes the e2e-ui `bucket-lifecycle` + `multipart-watchdog` failures. (commit `c6cde3b`)
