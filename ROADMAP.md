...
- **P2 — Env-driven multi-cluster S3 data backend.** `internal/data/s3/backend.go` is single-bucket-per-instance: `Backend{bucket, client, uploader, opTimeout, sseMode, sseKMSKeyID}` carries one S3 endpoint + one bucket. The RADOS backend went multi-cluster in US-044 (`STRATA_RADOS_CLUSTERS` + per-class `ClassSpec.Cluster` routing); S3 has no equivalent. Fix scope: mirror the RADOS env shape — `STRATA_S3_CLUSTERS` env (comma-separated `<id>:<endpoint>:<region>:<credentials_ref>` with bucket-less specs), new `S3ClusterSpec` struct, lift `Backend` to `clusters map[string]*s3Cluster` with per-cluster `*awss3.Client` + `*manager.Uploader` lazy-built via `connFor` on first use; new `ClassSpec{Cluster, Bucket}` (both REQUIRED) parsed from `STRATA_S3_CLASSES` env; bucket arg routing through `resolveClass` on every data-plane method (Put / Get / Delete / Head / Copy / List / PutChunks / GetChunks / Presign / Multipart); credentials never plaintext — discriminator (`chain` / `env:VAR1:VAR2` / `file:/path[:profile]`). Adding / removing a cluster requires gateway restart, per the Won't-do decision above. NOT in scope: chunk-side data migration between S3 clusters (covered by 'Per-bucket placement policy + rebalance worker' entry below).
