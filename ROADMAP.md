...
- **P3 — `bench-rgw-comparison` full 144-row sweep blocked by lima reference dev box capacity.** Surfaced by the `ralph/p1-fixes` cycle (US-007..US-008): the bench scaffold ships correct post-fix, but the lima dev box (9.7 GB shared across strata-a/b + ceph + ceph-b + tikv + pd + rgw + grafana/prometheus/nginx) saturates at concurrency ≥ 8 with both strata + rgw stacks coexisting — RGW degrades to 363 s warp timeouts on 1 MiB+ workloads. Strata stays measurable across the c=1/8/32/128 sweep; RGW only completes c=1 cleanly. Cells in `docs/site/content/architecture/benchmarks/rgw-comparison.md` carry honest `_pending — lima envelope blocked the run_` markers; the headline bucket-index verdict was reached via two boundary signals the lima box CAN measure (`put-small` c=8 + `list-100k` completes-vs-RGW-cannot) rather than fabricating numbers. Follow-up scope: (a) optional bench-harness mode `--target=strata-only` for capacity-constrained dev boxes (keeps saturation-removal evidence loop alive); (b) document the lima envelope as "single-workload at a time, c ≤ 8" in the bench README; (c) move full-sweep validation to a dedicated CI runner with ≥ 32 GiB RAM + NVMe OSDs to land the remaining head-to-head ratios on the `_pending` cells.
