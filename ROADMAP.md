...
- **P3 — Pools table shows class routing config, not actual cluster distribution.** `internal/data/rados/health.go::DataHealth` (and the S3 equivalent) iterates `Backend.classes` and emits one `PoolStatus` per class — `Cluster` field is sourced from the class env mapping (`STRATA_RADOS_CLASSES=STANDARD=pool@cluster`). In a symmetric multi-cluster deployment (same pools on every cluster, routing decided per-bucket via `Placement` policy), the UI shows N rows under `default` (the class env target) even though chunks are split N-ways across clusters via policy. Fix scope: change DataHealth to iterate `(cluster, distinct-pool) × ClusterStatsProbe.GetPoolStats(pool)` matrix so the wire returns one row per `(cluster, pool)` pair (e.g. 2 clusters × 3 pools = 6 rows); UI Pools table groups by cluster column (already exists since placement-ui US-002) and the "Bytes used" column reflects actual per-cluster footprint. Backend.classes still drives the class label per row (multiple classes mapping to the same pool aggregate to a comma-joined list, same shape memory + S3 backends already use). Operator can now see the true distribution that drives drain decisions.
- **P3 — Drain strict mode for PUT routing fallback.** Current drain semantics is fail-open: `placement.PickClusterExcluding` correctly skips draining clusters when walking the policy weight wheel, but if the picker returns `""` (empty/nil policy OR every clustered referenced by the policy is draining), the RADOS + S3 `PutChunks` paths fall back to `spec.Cluster` (the class default from `STRATA_RADOS_CLASSES` / `STRATA_S3_CLASSES`) **without consulting the drain map**. Two real scenarios bypass drain: (a) bucket has no `Placement` policy + the storage class is mapped to a draining cluster via env (e.g. `STANDARD_IA=...@cephb` while `cephb` is draining) → PUT lands on `cephb`; (b) bucket has a `Placement` policy that names only draining clusters (e.g. `{cephb:1}` while `cephb` is draining) → fallback hits class default which may itself be draining. This is intentional fail-open (data availability > drain strictness), but operators expecting "drain = no new chunks land here" need an opt-in strict mode. Fix scope: new env `STRATA_DRAIN_STRICT` (default `off` for backwards compat), when `on` makes RADOS + S3 `PutChunks` refuse to fall back to a draining cluster — return `503 ServiceUnavailable` with `code=DrainRefused` and `Retry-After: <minutes>` header so AWS-SDK clients backoff. Surface in the operator runbook + console (chip on cluster card showing "strict drain blocks PUTs"). Metric `strata_putchunks_refused_total{reason="drain_strict",cluster}` increments per refusal. ~3 stories: ctx-threaded drain check on the fallback path; new env + 503 wire shape; docs + UI chip + ROADMAP close-flip.
