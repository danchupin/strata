...
- **P2 — Uniform-by-default routing + `pending` cluster state (safe new-cluster activation).** Today a bucket without `meta.Bucket.Placement` policy falls back to `spec.Cluster` (class env default) — a single cluster regardless of how many are registered. Two consequences in multi-cluster deploys: (1) symmetric pool layout across clusters stays imbalanced — operator must set Placement on every bucket to get spread; (2) adding a new cluster to env triggers no UX safety — if we change default-routing to uniform-live, new clusters start receiving writes the moment they appear in `STRATA_RADOS_CLUSTERS` env, before operator has validated them. Fix scope: (a) extend state machine with new `pending` state — newly registered clusters (no existing row in meta) auto-init as `pending`; operator validates + explicit `POST /admin/v1/clusters/{id}/activate` flips to `live`; (b) `placement.PickCluster` walks weight wheel ONLY over `state="live"` clusters — `pending` / `draining_readonly` / `evacuating` excluded uniformly; (c) when `policy == nil` AND class env has no `@cluster` pin → synthesise uniform-live policy `{<each-live>:1}` so the bucket auto-spreads across all live clusters; (d) class env `@cluster` suffix remains explicit pin (back-compat for operators who want single-cluster routing per class); (e) UI: `<Pending>` cluster card variant with badge + "Activate" button + confirm modal explaining new-write-routing; admin endpoint `POST /admin/v1/clusters/{id}/activate` with audit row + 409 if state already live; (f) migration on upgrade: existing clusters with chunks already on them get state="live" via one-shot admin endpoint OR auto-detect at boot via `bucket_stats` sum > 0 referencing the cluster. ~4 stories. NOTE: do NOT bundle into the running `ralph/drain-transparency` cycle — that cycle's state machine refactor is mid-flight (US-001 already passed); fold `pending` as a 5th state in a follow-up cycle after drain-transparency merges.
...
- **P3 — Drain transparency + drain/evacuate split (always-strict).** Follow-up to `ralph/drain-lifecycle`. Five gaps surfaced by operator walkthrough on the multi-cluster lab: (1) opt-in `STRATA_DRAIN_STRICT` env is confusing — drain should be unconditional, no toggle; (2) drain proceeds silently with stuck chunks (buckets with single-cluster policy referencing only the draining cluster, or no policy at all relying on class-env routing) — no UI surface flags them; (3) drain conflates two operator intents — "stop-writes for maintenance window" vs "full evacuate for decommission" — must be two distinct modes; (4) in-flight multipart sessions need a graceful contract — open uploads must complete on the draining cluster, only new writes refused; (5) no pre-drain analysis or bulk policy fix flow forces operator to inspect each bucket individually. Fix scope: 4-state machine `{live, draining_readonly, evacuating, removed}` + `POST /drain {mode}` action + `GET /drain-impact` pre-evacuate preview with per-bucket suggested policies + `<BulkPlacementFixDialog>` (per-bucket picker + Apply-uniform-to-all) + redesigned `<ConfirmDrainModal>` with mode picker + Submit blocked if stuck>0 + `<DrainProgressBar>` showing categorized counts (migratable / stuck single-policy / stuck no-policy) + always-strict refactor (remove env, field, chip) + multipart graceful integration test + 3-scenario smoke (stop-writes / full-evacuate / upgrade readonly→evacuate). Also retro-notes the prior closed P3 "Drain strict mode" with env-removal addendum.
