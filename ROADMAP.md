...
- ~~**P2 — Default-routing weights + `pending` cluster state (safe gradual cluster activation).**~~ — **Done.** `ralph/cluster-weights` (US-001..US-005) shipped the 5th state `pending` + per-cluster `weight int (0..100)` field, boot-time reconcile that auto-inits new env clusters to `pending`/`0` (or `live`/`100` when `bucket_stats` already references them), `placement.DefaultPolicy` synthesised default routing, admin endpoints `POST /clusters/{id}/activate` + `PUT /clusters/{id}/weight` with audit + 409 transitions, `<ActivateClusterModal>` (typed-confirm + initial-weight slider) and `<LiveClusterWeightSlider>` (debounced PUT + weight=0 chip + 4xx revert), e2e Playwright spec + `make smoke-cluster-weights` four-scenario harness, and the `docs/site/content/best-practices/placement-rebalance.md` Cluster lifecycle section. Two-weight-layer rule enforced: bucket `Placement` always wins over cluster `weight`. (commit `<pending>`)
